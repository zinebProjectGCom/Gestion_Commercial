...
@section('content')
    ...
    <a href="{{ route('mode_reglements.create') }}" class="btn btn-primary">Add New Mode Reglement</a>
    <table class="table">
        <thead>
            <tr>
                <th>ID</th>
        ...
        <tbody>
            @foreach ($modeReglements as $modeReglement)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('mode_reglements.edit', $modeReglement->id) }}" class="btn btn-warning">Edit</a>
                        <form action="{{ route('mode_reglements.destroy', $modeReglement->id) }}" method="POST"
                            style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger"
                                onclick="return confirm('Are you sure you want to delete this mode reglement?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @endforeach
